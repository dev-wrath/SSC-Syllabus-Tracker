...
export const Dashboard: React.FC<DashboardProps> = ({ progressHistory, stats }) => {
  ...
  const StatCard: React.FC<{ label: string; value: number; color: string }> = ({ label, value, color }) => (
    <div className={`bg-slate-100 dark:bg-slate-800 p-4 rounded-lg flex flex-col items-center justify-center border-l-4 ${color}`}>
  ...
  return (
    <div className="bg-white dark:bg-slate-800/50 rounded-xl p-6 shadow-lg border border-slate-200 dark:border-slate-700">
      <h2 className="text-2xl font-bold mb-4 text-slate-800 dark:text-slate-100">Progress Dashboard</h2>
      
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          <span className="font-semibold">Overall Completion</span>
          <span className="font-bold text-cyan-500 dark:text-cyan-400">{stats.overallCompletion}%</span>
        </div>
        ...
        <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex space-x-1 bg-slate-200 dark:bg-slate-700 p-1 rounded-lg">
                {(['daily', 'weekly', 'monthly'] as Period[]).map(p => (
                    <button key={p} onClick={() => setPeriod(p)} className={`px-3 py-1 text-sm rounded-md capitalize transition-colors ${period === p ? 'bg-cyan-500 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600'}`}>
                        {p}
                    </button>
                ))}
            </div>
